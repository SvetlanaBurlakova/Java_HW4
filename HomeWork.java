...
public class HomeWork {
    ...
    public static void main(String[] args) throws ParseException, IOException{
        // 1. ѕусть дан LinkedList с несколькими элементами. –еализуйте метод, который вернЄет ЂперевЄрнутыйї список.
        List<Integer> list = new LinkedList<Integer>();
        Random rand = new Random();
        for (int i = 0; i < 20; i++) {
            list.add(rand.nextInt(15));
        }
        System.out.println("ѕервоначальный список:");
        System.out.println(list.toString());
        System.out.println("ѕеревернутый список");
        ReversePrint(list);
        System.out.println(list.toString());
        /* 2. –еализуйте очередь с помощью LinkedList со следующими методами:
        ...
        MyQueue(list);
        // 3. ¬ калькул€тор добавьте возможность отменить последнюю операцию.
        Calculator();
    ...
    public static void MyQueue(List<Integer> list) {
        ...
        int num = dequeue(list); //возвращает первый элемент из очереди и удал€ет его,
        System.out.printf("%d - первый элемент очереди, удален из списка", num);
        System.out.println();
        System.out.println(list.toString());

        int num2 = first(list); //возвращает первый элемент из очереди, не удал€€.
    ...
    public static List<Integer> enqueue(List<Integer> list) {
        ...
        System.out.println("¬ведите число,которое необходимо поместить в конец списка: ");
    ...
    try {
        ...
        while (!isCont.equals("no")) {
            if (stack.isEmpty()){
                System.out.print("¬ведите 1-ое число: ");
                ...
                if (stack.size() == 1) {
                    ...
                    System.out.print("¬ведите операцию (+, -, *, /): ");
                    System.out.println("ƒл€ отмены предыдущей операции, введите return: ");   
                    operation = iScanner.nextLine(); 
                } 
            ...
            if (operation.equals("+")) {
                ...
                System.out.println("Ќеправильно введена операци€");
                logger.log(Level.INFO, "Operation is not supported");
            }
            System.out.println();
    
            System.out.print("¬ы хотите выполнить еще одну операцию (yes/no): ");
